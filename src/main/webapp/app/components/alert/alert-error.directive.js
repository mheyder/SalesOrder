(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('salesOrderApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-salesOrderApp-error');
                var entityKey = httpResponse.headers('X-salesOrderApp-params');
                if (errorHeader) {
